...
| $P_{ m max}$                                | User max price willingness                                                                   |  |  |  |
| $TPS_{\mathrm{SLO}}$                        | User SLO Requirement                                                                         |  |  |  |
| Model Parameters                            |                                                                                              |  |  |  |
| Variable                                    | Description and Formula                                                                      |  |  |  |
...
| $BW_{gpu 	o cpu}$                           | Bandwidth for GPU-to-CPU data transfer                                                       |  |  |  |
...
- $\label{eq:weights: GPU} \begin{aligned} &\text{weights:} & &\text{GPU}_{\text{memory}}^i < \text{Mem}_{\text{model}}. \\ &\bullet &\text{If the KV Cache size per layer exceeds the available} \end{aligned}$ memory:  $Mem_{KVcache, per\_layer} > Mem_{avail}^{l}$
...
• **Decode Stage:** This stage generates each output token sequentially. The processing time per layer ( $T_{\rm dec}$ ) is multiplied by the number of layers (n) and the number of generated tokens ( $L_{\rm out}-1$ ), since the first output token is already processed in the Prefill stage.
...
The computation time  $T^p_{\mathrm{compute}}$  in the Prefill stage is divided into Linear Layer computation and Self-Attention computation, both of which are calculated by dividing the required floating-point operations (FLOPs) by the GPU's theoretical FLOPS capacity:
...
The KV Cache transfer time  $T_{\rm trans}^p$  in the Prefill stage represents the time required to offload the generated KV Cache from GPU to CPU memory and is computed as:
...
$$T_{\text{dec}} = CTCF(T_{\text{compute}}^{\text{d}}) + T_{\text{trans}}^{\text{d}}$$
 (6)
...
$$T_{\rm trans}^d = \frac{(C_{\rm off} \cdot 2 \cdot (L_{\rm in} + 1) + L_{\rm out}) \cdot h_1 \cdot \text{Precision}_{\rm bytes} \cdot BS}{\text{BW}_{\rm cpu} \rightarrow \text{gpu}}$$
...
First, instances that fail to satisfy the user-defined SLO constraint (TPS  $\geq$  TPS<sub>SLO</sub>) are eliminated from consideration. Next, the cost efficiency metric (Equation 3) is calculated for each remaining instance. Finally, the instance with the highest cost efficiency is selected. In the event of a tie, the instance with the higher TPS is prioritized.
